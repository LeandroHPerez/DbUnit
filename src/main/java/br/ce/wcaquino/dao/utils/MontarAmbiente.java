...
public class MontarAmbiente {
	...
	private static void removerEstuturas() throws ClassNotFoundException, SQLException{
		...
		conn.createStatement().executeUpdate("ALTER TABLE public.usuarios DROP CONSTRAINT IF EXISTS conta_principal_id_foreign;"); //dropa a constraint do relacionamento ciclico
	...
	private static void criarEstuturas() throws ClassNotFoundException, SQLException{
		...
		conn.createStatement().executeUpdate("ALTER TABLE public.usuarios ADD CONSTRAINT conta_principal_id_foreign FOREIGN KEY (conta_principal_id) REFERENCES public.contas (id) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE NO ACTION "); //adiciona a constraint do relacionamento ciclico
		ConnectionFactory.closeConnection();
	}
